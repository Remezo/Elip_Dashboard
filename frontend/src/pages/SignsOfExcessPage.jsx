...
export default function SignsOfExcessPage() {
  const [data, setData] = useState(null);
  const [error, setError] = useState(null);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    getSectorCharts("Signs of Excess")
      .then(setData)
      .catch((err) => setError(err.message))
      .finally(() => setLoading(false));
  }, []);

  if (loading) return <LoadingSpinner message="Loading Signs of Excess..." />;
  ...
  return (
    <div>
      <div className="border-l-4 border-brand-gold pl-3 mb-6">
        <h2 className="text-lg md:text-xl font-bold text-brand-white tracking-wide uppercase">Signs of Excess</h2>
      </div>
